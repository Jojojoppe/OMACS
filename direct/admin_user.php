...
<div class="card mt-2">
	...
	<div class="card-body">
				...
				if(!userIdExists($userId)){
				  Redirect::to($us_url_root.$VIEWNABASE.'users&err=That user does not exist.'); die();
				}
				...
				if(!empty($_POST)) {
				  ...
				  if(!empty($_POST['delete'])){
					...
					if ($deletion_count = deleteUsers($deletions)){
					  logger($user->data()->id,"User Manager","Deleted user named $userdetails->fname.");
								Redirect::to($us_url_root.$VIEWNABASE.'users&msg='.lang("ACCOUNT_DELETIONS_SUCCESSFUL", array($deletion_count)));
					}
					else {
				  ...
				  if($user->data()->cloak_allowed!=1 && !in_array($user->data()->id,$master_account) && !isset($_SESSION['cloak_to'])) {
					...
					Redirect::to($VIEWNABASE.'users&err=You do not have permission to cloak.');
				  }else{
					if(in_array($userId,$master_account) && !in_array($user->data()->id,$master_account)){
					  ...
					  Redirect::to($VIEWNABASE.'users&err=You cannot cloak into a master account.');
					}elseif($userId == $user->data()->id){
					  logger($user->data()->id,"Cloaking","User attempted to cloak themself.");
					  Redirect::to($VIEWNABASE.'users&err=Cloaking+into+yourself+would+open+up+a+black+hole!');
					}else{
					  $check = $db->query("SELECT id FROM users WHERE id = ?",array($userId));
					  $count = $check->count();
					  if($count < 1){
						Redirect::to($VIEWNABASE.'users&err=You+broke+it!+User+not+found.');
					  }else{
						$_SESSION['cloak_from']=$user->data()->id;
						$_SESSION['cloak_to']=$userId;
						logger($user->data()->id,"Cloaking","Cloaked into ".$userId);
						Redirect::to($VIEWNABASE.'profile&err=You+are+now+cloaked!');
					  }
					}
				  }
				}
					...
					if (empty($errors)) {
					  ...
					  $db->update('users',$userdetails->id,['vericode' => $vericode,'vericode_expiry' => $vericode_expiry]);
						if(isset($_POST['sendPwReset'])) {
						  ...
						  $body = email_body('../../email_templates/adminPwReset.php',$params);
				...
				<div id="misc" class="modal fade" role="dialog">
				  <div class="modal-dialog">
					...
					<div class="modal-content">
					  ...
					  <input type='checkbox' name='delete[<?php echo "$userId"; ?>]' id='delete[<? echo "$userId"; ?>]' value='<?php echo "$userId"; ?>'></label>
					  </div>
					  <div class="modal-footer">
						  <div class="btn-group"><input class='btn btn-primary' type='submit' value='Update' class='submit' /></div>
						 <div class="btn-group"><button type="button" class="btn btn-default" data-dismiss="modal">Close</button></div>
						...
						<input type="hidden" name="csrf" value="<?=Token::generate();?>" />
								<div class="pull-right">
										<div class="btn-group"><input class='btn btn-primary' type='submit' value='Update' class='submit' /></div>
										<div class="btn-group"><a class='btn btn-warning' href="<?=$VIEWNABASE?>users">Cancel</a></div><br /><Br />
								</div>
